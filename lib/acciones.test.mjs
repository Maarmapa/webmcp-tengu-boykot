...
test('los hooks solo se montan si el navegador trae WebMCP', () => {
  ...
  // `soportado` lo decide el efecto DESPUÉS de resolver la superficie (que con
  // el flag de demo puede aparecer recién al cargar el polyfill): si no hay
  // superficie, se retorna antes de setSoportado y las acciones no se montan.
  assert.match(webmcp, /if \(!mc \|\| !vivo\) return;/);
  assert.match(webmcp, /setSoportado\(true\)/);
